package adminapi

import (
	"encoding/json"
	"io"
	"log/slog"
	"net/http"
	"net/http/httptest"
	"os"
	"path/filepath"
	"strconv"
	"testing"

	"loadout/core/config"
	"loadout/core/db"
	"loadout/core/store"
	"loadout/plugins/admin-auth"
	"loadout/plugins/gateway-keys"
	"loadout/plugins/skills"
	unifyai "loadout/plugins/unifyai"
	"loadout/plugins/types"
)

// newDBTestServer 组装带 DB（routing 非 nil）的完整服务，用于测 DB 版渠道模型编辑。
func newDBTestServer(t *testing.T) (*httptest.Server, *store.Store, string) {
	t.Helper()
	dir := t.TempDir()
	st, err := store.New(dir)
	if err != nil {
		t.Fatalf("store.New: %v", err)
	}

	old := config.AdminPasswordFile
	config.AdminPasswordFile = filepath.Join(dir, "admin-password")
	t.Cleanup(func() { config.AdminPasswordFile = old })

	authSvc := adminauth.NewService(st, slog.Default())
	if _, err := authSvc.EnsureFirstRun(); err != nil {
		t.Fatalf("EnsureFirstRun: %v", err)
	}
	pw, err := os.ReadFile(config.AdminPasswordFile)
	if err != nil {
		t.Fatalf("读取初始密码: %v", err)
	}

	database, err := db.OpenForStore(st)
	if err != nil {
		t.Fatalf("db.OpenForStore: %v", err)
	}
	t.Cleanup(func() { _ = database.Close() })
	routing, err := db.NewRepository(database)
	if err != nil {
		t.Fatalf("db.NewRepository: %v", err)
	}

	keys := gatewaykeys.NewManager(st)
	skillSvc := skills.NewService(st, slog.Default(), t.TempDir(), t.TempDir())
	svc := NewService(st, slog.Default(), authSvc, keys, skillSvc, nil, unifyai.NewService(slog.Default()))
	svc.SetRoutingServices(database, routing, nil, nil)

	ts := httptest.NewServer(svc.Handler())
	t.Cleanup(ts.Close)
	return ts, st, string(pw)
}

// TestChannelModelsReplaceDB DB 版编辑接口：添加/禁用/删除 + source 保留。
func TestChannelModelsReplaceDB(t *testing.T) {
	ts, _, pw := newDBTestServer(t)
	cookie := login(t, ts, pw)

	// 创建渠道：base_url 不可达 → 探测失败，Models 为空（正是用户场景）。
	resp, data := apiReq(t, ts, http.MethodPost, "/api/channels", map[string]any{
		"name":     "自定义渠道",
		"base_url": "http://127.0.0.1:1/v1", // 不可达，探测必失败
		"enabled":  true,
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("创建渠道失败: %d %s", resp.StatusCode, data)
	}
	var created types.Channel
	if err := json.Unmarshal(data, &created); err != nil {
		t.Fatalf("解析创建响应: %v", err)
	}

	// 编辑接口：添加 2 个手动模型（一个禁用 = 删除，不写入目录）。
	resp, data = apiReq(t, ts, http.MethodPut, "/api/channels/"+created.ID+"/models", []map[string]any{
		{"model": "my-custom-v1", "enabled": true},
		{"model": "old-model", "enabled": false},
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("编辑模型期望 200，实际 %d: %s", resp.StatusCode, data)
	}

	// 渠道详情应只有 1 个模型（enabled=false 的 old-model 视为删除），source=manual。
	_, data = apiReq(t, ts, http.MethodGet, "/api/channels", nil, cookie)
	var list []types.Channel
	if err := json.Unmarshal(data, &list); err != nil {
		t.Fatalf("解析渠道列表: %v", err)
	}
	var found *types.Channel
	for i := range list {
		if list[i].ID == created.ID {
			found = &list[i]
			break
		}
	}
	if found == nil {
		t.Fatalf("渠道不存在")
	}
	if len(found.ModelsDetail) != 1 {
		t.Fatalf("detail 应只有 1 个启用模型，实际 %+v", found.ModelsDetail)
	}
	detail := map[string]types.ChannelModelDetail{}
	for _, m := range found.ModelsDetail {
		detail[m.Model] = m
	}
	if detail["my-custom-v1"].Source != "manual" || !detail["my-custom-v1"].Enabled {
		t.Fatalf("my-custom-v1 应为 manual 且启用: %+v", detail["my-custom-v1"])
	}
	if _, ok := detail["old-model"]; ok {
		t.Fatalf("old-model（enabled=false）应被删除，实际仍在目录: %+v", detail["old-model"])
	}
	// 兼容字段 Models 与 detail 一致（只含启用的）。
	if len(found.Models) != 1 || found.Models[0] != "my-custom-v1" {
		t.Fatalf("兼容字段 Models 应只含启用的: %+v", found.Models)
	}

	// 再编辑：新增第三个模型，old-model 保持删除。
	resp, _ = apiReq(t, ts, http.MethodPut, "/api/channels/"+created.ID+"/models", []map[string]any{
		{"model": "my-custom-v1", "enabled": true},
		{"model": "brand-new", "enabled": true},
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("二次编辑期望 200，实际 %d", resp.StatusCode)
	}
	_, data = apiReq(t, ts, http.MethodGet, "/api/channels", nil, cookie)
	if err := json.Unmarshal(data, &list); err != nil {
		t.Fatalf("解析渠道列表: %v", err)
	}
	for i := range list {
		if list[i].ID == created.ID {
			found = &list[i]
			break
		}
	}
	if len(found.ModelsDetail) != 2 {
		t.Fatalf("二次编辑后 detail 应有 2 个模型，实际 %+v", found.ModelsDetail)
	}
	models := map[string]bool{}
	for _, m := range found.ModelsDetail {
		models[m.Model] = m.Enabled
	}
	if models["old-model"] {
		t.Fatalf("old-model 应保持删除")
	}
	if !models["my-custom-v1"] || !models["brand-new"] {
		t.Fatalf("my-custom-v1/brand-new 应存在且启用: %+v", models)
	}
}

// TestChannelUpdateDBKeepsCatalog 编辑已有目录的渠道不得重新探测上游
// （base_url 不可达 → 若探测会把目录清空或污染）。模型目录只由
// PUT /models 显式管理；用户编辑渠道元数据（名称等）不影响模型清单。
func TestChannelUpdateDBKeepsCatalog(t *testing.T) {
	ts, _, pw := newDBTestServer(t)
	cookie := login(t, ts, pw)

	// 创建渠道（base_url 不可达 → 探测失败，Models 为空）。
	resp, data := apiReq(t, ts, http.MethodPost, "/api/channels", map[string]any{
		"name":     "keep-catalog",
		"base_url": "http://127.0.0.1:1/v1",
		"enabled":  true,
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("创建渠道失败: %d %s", resp.StatusCode, data)
	}
	var created types.Channel
	if err := json.Unmarshal(data, &created); err != nil {
		t.Fatalf("解析创建响应: %v", err)
	}

	// 手动设置目录：2 个启用模型。
	resp, data = apiReq(t, ts, http.MethodPut, "/api/channels/"+created.ID+"/models", []map[string]any{
		{"model": "keep-a", "enabled": true},
		{"model": "keep-b", "enabled": true},
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("设置模型期望 200，实际 %d: %s", resp.StatusCode, data)
	}

	// 编辑渠道元数据（只改名称，不改 base_url），带模型列表 → 不得触发探测。
	resp, data = apiReq(t, ts, http.MethodPut, "/api/channels/"+created.ID, map[string]any{
		"name":     "keep-catalog-renamed",
		"base_url": "http://127.0.0.1:1/v1",
		"enabled":  true,
		"models":   []string{"keep-a", "keep-b"},
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("编辑渠道期望 200，实际 %d: %s", resp.StatusCode, data)
	}

	// 目录必须保持 2 个模型，不能被探测污染/清空。
	_, data = apiReq(t, ts, http.MethodGet, "/api/channels", nil, cookie)
	var list []types.Channel
	if err := json.Unmarshal(data, &list); err != nil {
		t.Fatalf("解析渠道列表: %v", err)
	}
	var found *types.Channel
	for i := range list {
		if list[i].ID == created.ID {
			found = &list[i]
			break
		}
	}
	if found == nil {
		t.Fatalf("渠道不存在")
	}
	if len(found.ModelsDetail) != 2 {
		t.Fatalf("编辑后目录应保持 2 个模型（不重新探测），实际 %+v", found.ModelsDetail)
	}
	if found.Name != "keep-catalog-renamed" {
		t.Fatalf("名称应更新为 keep-catalog-renamed，实际 %q", found.Name)
	}
}

// TestChannelCreateWithModelsSkipsProbe 创建时请求里带了模型列表 → 不得请求上游探测，
// 直接用调用方提供的列表（source=manual）。
func TestChannelCreateWithModelsSkipsProbe(t *testing.T) {
	ts, _, pw := newDBTestServer(t)
	cookie := login(t, ts, pw)

	// 假上游：记录 /v1/models 被请求的次数（期望 0 次）。
	probeHits := 0
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/models" {
			http.NotFound(w, r)
			return
		}
		probeHits++
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"object":"list","data":[{"id":"upstream-a"}]}`)
	}))
	t.Cleanup(upstream.Close)

	resp, data := apiReq(t, ts, http.MethodPost, "/api/channels", map[string]any{
		"name":     "create-with-models",
		"base_url": upstream.URL + "/v1",
		"api_key":  "sk-test",
		"enabled":  true,
		"models":   []string{"my-a", "my-b"},
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("创建渠道失败: %d %s", resp.StatusCode, data)
	}
	var created types.Channel
	if err := json.Unmarshal(data, &created); err != nil {
		t.Fatalf("解析创建响应: %v", err)
	}
	if probeHits != 0 {
		t.Fatalf("提供模型列表后创建不应请求上游探测，实际探测 %d 次", probeHits)
	}
	if len(created.ModelsDetail) != 2 {
		t.Fatalf("创建后模型列表应为 2 个（调用方提供），实际 %+v", created.ModelsDetail)
	}
	for _, m := range created.ModelsDetail {
		if m.Source != "manual" {
			t.Fatalf("调用方提供的模型应标记 manual，实际 %+v", m)
		}
	}
}

// TestChannelModelsClearAutoProbe 删空模型清单后保存 → 后台自动重新探测填充，
// 渠道不应晾在无模型状态（UI 显示「0 个/未知」）。
func TestChannelModelsClearAutoProbe(t *testing.T) {
	ts, _, pw := newDBTestServer(t)
	cookie := login(t, ts, pw)

	// 假上游：/v1/models 返回 2 个模型。
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/models" {
			http.NotFound(w, r)
			return
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"object":"list","data":[{"id":"auto-a"},{"id":"auto-b"}]}`)
	}))
	t.Cleanup(upstream.Close)

	// 创建渠道（探测成功：目录 = auto-a, auto-b）。
	resp, data := apiReq(t, ts, http.MethodPost, "/api/channels", map[string]any{
		"name":     "clear-probe",
		"base_url": upstream.URL + "/v1",
		"api_key":  "sk-test",
		"enabled":  true,
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("创建渠道失败: %d %s", resp.StatusCode, data)
	}
	var created types.Channel
	if err := json.Unmarshal(data, &created); err != nil {
		t.Fatalf("解析创建响应: %v", err)
	}

	// 删除全部模型（enabled=false）→ 保存。
	resp, data = apiReq(t, ts, http.MethodPut, "/api/channels/"+created.ID+"/models", []map[string]any{
		{"model": "auto-a", "enabled": false},
		{"model": "auto-b", "enabled": false},
	}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("删空模型期望 200，实际 %d: %s", resp.StatusCode, data)
	}

	// 目录应被自动探测重新填充为 2 个模型。
	_, data = apiReq(t, ts, http.MethodGet, "/api/channels", nil, cookie)
	var list []types.Channel
	if err := json.Unmarshal(data, &list); err != nil {
		t.Fatalf("解析渠道列表: %v", err)
	}
	var found *types.Channel
	for i := range list {
		if list[i].ID == created.ID {
			found = &list[i]
			break
		}
	}
	if found == nil {
		t.Fatalf("渠道不存在")
	}
	if len(found.ModelsDetail) != 2 {
		t.Fatalf("删空后应自动探测填充 2 个模型，实际 %+v", found.ModelsDetail)
	}
	if found.ModelsError != "" {
		t.Fatalf("探测成功后 models_error 应为空，实际 %q", found.ModelsError)
	}
}

// TestChannelsReorder 全量重排渠道顺序：支撑前端按 base_url 分组后的整组移动。
func TestChannelsReorder(t *testing.T) {
	ts, _, pw := newDBTestServer(t)
	cookie := login(t, ts, pw)

	// 建 3 个渠道（探测不可达，Models 为空，无影响）。
	var ids []string
	for _, name := range []string{"A", "B", "C"} {
		resp, data := apiReq(t, ts, http.MethodPost, "/api/channels", map[string]any{
			"name":     name,
			"base_url": "http://127.0.0.1:1/v1", // 不可达，探测必失败
			"enabled":  true,
		}, cookie)
		if resp.StatusCode != http.StatusOK {
			t.Fatalf("创建渠道 %s 失败: %d %s", name, resp.StatusCode, data)
		}
		var created types.Channel
		if err := json.Unmarshal(data, &created); err != nil {
			t.Fatalf("解析创建响应: %v", err)
		}
		ids = append(ids, created.ID)
	}
	channelOrder := func() []string {
		_, data := apiReq(t, ts, http.MethodGet, "/api/channels", nil, cookie)
		var list []types.Channel
		if err := json.Unmarshal(data, &list); err != nil {
			t.Fatalf("解析渠道列表: %v", err)
		}
		out := make([]string, 0, len(list))
		for _, ch := range list {
			out = append(out, ch.ID)
		}
		return out
	}

	// 乱序提交：[C, A, B] → 顺序应为 C, A, B。
	resp, data := apiReq(t, ts, http.MethodPost, "/api/channels/reorder",
		map[string]any{"ids": []string{ids[2], ids[0], ids[1]}}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("reorder 期望 200，实际 %d: %s", resp.StatusCode, data)
	}
	got := channelOrder()
	want := []string{ids[2], ids[0], ids[1]}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("reorder 后顺序 %v，期望 %v", got, want)
		}
	}

	// 部分提交 + 未知 id：未知忽略，未提交的追加尾部（保持原相对顺序）。
	resp, data = apiReq(t, ts, http.MethodPost, "/api/channels/reorder",
		map[string]any{"ids": []string{"unknown-id", ids[2]}}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("reorder(部分) 期望 200，实际 %d: %s", resp.StatusCode, data)
	}
	got = channelOrder()
	want = []string{ids[2], ids[0], ids[1]}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("部分提交后顺序 %v，期望 %v", got, want)
		}
	}

	// 完整乱序再提交一次，验证幂等。
	resp, data = apiReq(t, ts, http.MethodPost, "/api/channels/reorder",
		map[string]any{"ids": []string{ids[1], ids[2], ids[0]}}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("reorder(完整) 期望 200，实际 %d: %s", resp.StatusCode, data)
	}
	got = channelOrder()
	want = []string{ids[1], ids[2], ids[0]}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("完整提交后顺序 %v，期望 %v", got, want)
		}
	}

	// 重复 id：去重不报错，顺序不变。
	resp, data = apiReq(t, ts, http.MethodPost, "/api/channels/reorder",
		map[string]any{"ids": []string{ids[1], ids[1], ids[2], ids[0]}}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("reorder(重复) 期望 200，实际 %d: %s", resp.StatusCode, data)
	}
	got = channelOrder()
	want = []string{ids[1], ids[2], ids[0]}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("重复提交后顺序 %v，期望 %v", got, want)
		}
	}

	// 空 ids：不改变任何顺序，也不报错。
	resp, data = apiReq(t, ts, http.MethodPost, "/api/channels/reorder",
		map[string]any{"ids": []string{}}, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("reorder(空) 期望 200，实际 %d: %s", resp.StatusCode, data)
	}
	got = channelOrder()
	want = []string{ids[1], ids[2], ids[0]}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("空提交后顺序 %v，期望 %v", got, want)
		}
	}
}

// TestMergeManualModels 探测合并：manual 保留，probe 全量替换。
func TestMergeManualModels(t *testing.T) {
	now := "2026-08-18T00:00:00Z"
	existing := []db.ChannelModel{
		{Model: "m-manual", Source: "manual", Enabled: true},
		{Model: "m-probe-old", Source: "probe", Enabled: true},
	}
	probed := []db.ChannelModel{
		{Model: "m-probe-old", Source: "probe", Enabled: true},
		{Model: "m-probe-new", Source: "probe", Enabled: true},
	}
	merged := mergeManualModels(existing, probed, now)

	names := map[string]bool{}
	for _, m := range merged {
		names[m.Model] = true
	}
	if !names["m-manual"] {
		t.Fatalf("手动模型应保留: %+v", names)
	}
	if !names["m-probe-old"] || !names["m-probe-new"] {
		t.Fatalf("探测模型应全量保留: %+v", names)
	}
	if len(merged) != 3 {
		t.Fatalf("合并后应 3 个模型，实际 %d: %+v", len(merged), merged)
	}
}

// TestMergeManualModelsProbeFailed 探测失败（空结果）时手动模型保留。
func TestMergeManualModelsProbeFailed(t *testing.T) {
	existing := []db.ChannelModel{
		{Model: "m-manual", Source: "manual", Enabled: true},
		{Model: "m-probe", Source: "probe", Enabled: true},
	}
	merged := mergeManualModels(existing, nil, "2026-08-18T00:00:00Z")
	if len(merged) != 1 || merged[0].Model != "m-manual" {
		t.Fatalf("探测失败应只保留 manual: %+v", merged)
	}
}

// TestOverviewChannelsDB DB 模式下概览渠道数应来自 sqlite 而非 channels.json。
// 回归：handleOverview 曾只读 JSON 文件，渠道迁库后恒为 0。
func TestOverviewChannelsDB(t *testing.T) {
	ts, st, pw := newDBTestServer(t)
	cookie := login(t, ts, pw)

	// DB 中创建 2 个渠道（走 handleChannelCreateDB，仅写入 sqlite，不落 channels.json）。
	for i := 0; i < 2; i++ {
		resp, data := apiReq(t, ts, http.MethodPost, "/api/channels", map[string]any{
			"name":     "db-channel-" + strconv.Itoa(i),
			"base_url": "http://127.0.0.1:1/v1", // 不可达，探测失败但创建成功
			"enabled":  true,
		}, cookie)
		if resp.StatusCode != http.StatusOK {
			t.Fatalf("创建渠道失败: %d %s", resp.StatusCode, data)
		}
	}

	// channels.json 应不存在（数据全在 DB）。
	if err := st.Read(types.FileChannels, &struct{}{}); err == nil {
		t.Fatal("DB 模式下 channels.json 不应存在")
	}

	// 概览渠道数应为 2。
	resp, data := apiReq(t, ts, http.MethodGet, "/api/overview", nil, cookie)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("overview 期望 200，实际 %d: %s", resp.StatusCode, data)
	}
	var overview struct {
		Channels int `json:"channels"`
	}
	if err := json.Unmarshal(data, &overview); err != nil {
		t.Fatalf("解析 overview: %v", err)
	}
	if overview.Channels != 2 {
		t.Fatalf("DB 模式概览渠道数应 2，实际 %d", overview.Channels)
	}
}
