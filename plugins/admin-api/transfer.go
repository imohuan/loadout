...
func (s *Service) writeExportSection(ctx context.Context, zw *zip.Writer, key string) error {
	...
	case sectionCapabilityRoutes:
		...
		if err := s.readJSONOrEmpty(types.FileCapabilityRoutes, &routes); err != nil {
			return err
		}
		return writeZipEntry(zw, "loadout-config/capability_routes.json", routes)
	case sectionMCP:
		var servers []types.MCPServer
		if err := s.readJSONOrEmpty(types.FileMCPServers, &servers); err != nil {
			return err
		}
		...
		if err := s.readJSONOrEmpty(types.FileGroups, &groups); err != nil {
			return err
		}
		...
		if err := s.readJSONOrEmpty(types.FileToolsState, &states); err != nil {
			return err
		}
		return writeZipEntry(zw, "loadout-config/mcp_tools_state.json", states)
	case sectionSkills:
		var skills []types.Skill
		if err := s.readJSONOrEmpty(types.FileSkills, &skills); err != nil {
			return err
		}
		...
		if err := s.readJSONOrEmpty(types.FilePresets, &presets); err != nil {
			return err
		}
		...
		if err := s.readJSONOrEmpty(types.FileSettings, &settings); err != nil {
			return err
		}
...
func (s *Service) handleConfigImport(w http.ResponseWriter, r *http.Request) {
	...
	for _, info := range transferSections {
		...
		result, err := s.applySection(ctx, info, entry.data, modes[info.key])
		if err != nil {
...
func (s *Service) applyExtraFile(ctx context.Context, key, name, file string, data []byte, mode string) (sectionResult, error) {
	...
	case "settings":
		...
		if err := s.st.Write(types.FileSettings, settings); err != nil {
			return result, err
		}
		result.Imported = 1
		return result, nil
