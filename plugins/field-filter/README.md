...
> `channel_base_urls` 按**精确匹配**（仅忽略尾斜杠）：与渠道表里配置的 base_url 完全一致才命中。
> 版本段（`/v1`、`/v2` 等）属于渠道的一部分，`/v1` 与 `/v2` 是不同的渠道——路由里配哪个版本段就命中哪个。
> 渠道 base_url 是你自己配置的、查询用的也是它，所以不存在"配了 v1 却命中 v2"的情况。
