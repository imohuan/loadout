...
func (s *Service) Attempt(ctx context.Context, attempt contracts.RouteAttempt) (int64, error) {
	...
	resultRow, err := s.db.ExecContext(ctx, `INSERT INTO route_attempts(request_id, previous_attempt_id, step_no, action, model, channel_id, channel_ids_json, channel_base_url, started_at, finished_at, result, failure_class, status_code, error_message, duration_ms, stream, prompt_tokens, completion_tokens, cached_tokens, metadata_json) VALUES (?, ?, ?, ?, ?, COALESCE(NULLIF(?, ''), ''), ?, COALESCE(NULLIF(?, ''), ''), ?, ?, ?, ?, NULLIF(?, 0), ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(request_id, step_no) DO UPDATE SET action=excluded.action, model=excluded.model, channel_id=excluded.channel_id, channel_ids_json=excluded.channel_ids_json, channel_base_url=excluded.channel_base_url, started_at=excluded.started_at, finished_at=excluded.finished_at, result=excluded.result, failure_class=excluded.failure_class, status_code=excluded.status_code, error_message=excluded.error_message, duration_ms=excluded.duration_ms, stream=excluded.stream, prompt_tokens=excluded.prompt_tokens, completion_tokens=excluded.completion_tokens, cached_tokens=excluded.cached_tokens, metadata_json=excluded.metadata_json`, attempt.RequestID, attempt.PreviousAttemptID, attempt.StepNo, action, attempt.Model, attempt.ChannelID, encodeStringSlice(attempt.ChannelIDs), attempt.ChannelBaseURL, attempt.StartedAt.UTC().Format(time.RFC3339Nano), finished, result, attempt.FailureClass, attempt.StatusCode, redact(attempt.ErrorMessage), attempt.Duration.Milliseconds(), boolToInt(attempt.Stream), attempt.PromptTokens, attempt.CompletionTokens, attempt.CachedTokens, metadata)
	if err != nil {
		return 0, err
	}
...
func (s *Service) Finish(ctx context.Context, finish contracts.RouteFinish) error {
	...
	_, err := s.db.ExecContext(ctx, `UPDATE route_requests SET finished_at=?, result=?, final_model=NULLIF(?, ''), final_channel_id=COALESCE(NULLIF(?, ''), ''), final_channel_ids_json=?, final_channel_base_url=COALESCE(NULLIF(?, ''), ''), http_status=NULLIF(?, 0), duration_ms=?, error_message=?, stream=?, prompt_tokens=?, completion_tokens=?, cached_tokens=? WHERE request_id=?`, finish.FinishedAt.UTC().Format(time.RFC3339Nano), finish.Result, finish.FinalModel, finish.FinalChannelID, encodeStringSlice(finish.FinalChannelIDs), finish.FinalChannelBaseURL, finish.HTTPStatus, finish.Duration.Milliseconds(), redact(finish.ErrorMessage), boolToInt(finish.Stream), finish.PromptTokens, finish.CompletionTokens, finish.CachedTokens, finish.RequestID)
	return err
}
...
func (s *Service) List(ctx context.Context, filter contracts.RouteLogFilter) ([]contracts.RouteRequestView, error) {
	query := `SELECT request_id, requested_model, COALESCE(virtual_model, ''), started_at, finished_at, result, COALESCE(final_model, ''), COALESCE(final_channel_id, ''), COALESCE(final_channel_ids_json, '[]'), COALESCE(final_channel_base_url, ''), COALESCE(http_status, 0), COALESCE(duration_ms, 0), error_message, COALESCE(stream, 0), COALESCE(prompt_tokens, 0), COALESCE(completion_tokens, 0), COALESCE(cached_tokens, 0) FROM route_requests r WHERE 1=1`
	args := []any{}
	if filter.Model != "" {
...
func (s *Service) Detail(ctx context.Context, requestID string) (contracts.RouteRequestView, error) {
	row := s.db.QueryRowContext(ctx, `SELECT request_id, requested_model, COALESCE(virtual_model, ''), started_at, finished_at, result, COALESCE(final_model, ''), COALESCE(final_channel_id, ''), COALESCE(final_channel_ids_json, '[]'), COALESCE(final_channel_base_url, ''), COALESCE(http_status, 0), COALESCE(duration_ms, 0), error_message, COALESCE(stream, 0), COALESCE(prompt_tokens, 0), COALESCE(completion_tokens, 0), COALESCE(cached_tokens, 0) FROM route_requests WHERE request_id=?`, requestID)
	view, err := scanRequest(row)
	if err != nil {
		return contracts.RouteRequestView{}, err
	}
	rows, err := s.db.QueryContext(ctx, `SELECT id, request_id, previous_attempt_id, step_no, action, model, COALESCE(channel_id, ''), COALESCE(channel_ids_json, '[]'), COALESCE(channel_base_url, ''), started_at, finished_at, result, failure_class, COALESCE(status_code, 0), error_message, COALESCE(duration_ms, 0), COALESCE(stream, 0), COALESCE(prompt_tokens, 0), COALESCE(completion_tokens, 0), COALESCE(cached_tokens, 0), metadata_json FROM route_attempts WHERE request_id=? ORDER BY step_no`, requestID)
	if err != nil {
		return contracts.RouteRequestView{}, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &attempt.RequestID, &previous, &attempt.StepNo, &attempt.Action, &attempt.Model, &attempt.ChannelID, &channelIDsJSON, &attempt.ChannelBaseURL, &started, &finished, &attempt.Result, &attempt.FailureClass, &attempt.StatusCode, &attempt.ErrorMessage, &duration, &stream, &promptTokens, &completionTokens, &cachedTokens, &metadata); err != nil {
			return contracts.RouteRequestView{}, err
		}
...
func scanRequest(scanner scanner) (contracts.RouteRequestView, error) {
	...
	if err := scanner.Scan(&view.RequestID, &view.RequestedModel, &view.VirtualModel, &started, &finished, &view.Result, &view.FinalModel, &view.FinalChannelID, &finalChannelIDsJSON, &view.FinalChannelBaseURL, &view.HTTPStatus, &duration, &view.ErrorMessage, &stream, &promptTokens, &completionTokens, &cachedTokens); err != nil {
		return view, err
	}
