...
func (s *Service) Attempt(ctx context.Context, attempt contracts.RouteAttempt) (int64, error) {
	...
	resultRow, err := s.db.ExecContext(ctx, `INSERT INTO route_attempts(request_id, previous_attempt_id, step_no, action, model, channel_id, channel_ids_json, channel_base_url, channel_name, started_at, finished_at, first_byte_at, result, failure_class, status_code, error_message, error_body, duration_ms, stream, prompt_tokens, completion_tokens, cached_tokens, metadata_json, request_log_id) VALUES (?, ?, ?, ?, ?, COALESCE(NULLIF(?, ''), ''), ?, COALESCE(NULLIF(?, ''), ''), COALESCE(NULLIF(?, ''), ''), ?, ?, ?, ?, ?, NULLIF(?, 0), ?, ?, ?, ?, ?, ?, ?, ?, COALESCE(NULLIF(?, ''), '')) ON CONFLICT(request_id, step_no) DO UPDATE SET action=excluded.action, model=excluded.model, channel_id=excluded.channel_id, channel_ids_json=excluded.channel_ids_json, channel_base_url=excluded.channel_base_url, channel_name=CASE WHEN excluded.channel_name = '' THEN route_attempts.channel_name ELSE excluded.channel_name END, started_at=excluded.started_at, finished_at=excluded.finished_at, first_byte_at=COALESCE(excluded.first_byte_at, route_attempts.first_byte_at), result=excluded.result, failure_class=excluded.failure_class, status_code=excluded.status_code, error_message=excluded.error_message, error_body=CASE WHEN excluded.error_body = '' THEN route_attempts.error_body ELSE excluded.error_body END, duration_ms=excluded.duration_ms, stream=excluded.stream, prompt_tokens=excluded.prompt_tokens, completion_tokens=excluded.completion_tokens, cached_tokens=excluded.cached_tokens, metadata_json=excluded.metadata_json, request_log_id=CASE WHEN excluded.request_log_id = '' THEN route_attempts.request_log_id ELSE excluded.request_log_id END`, attempt.RequestID, attempt.PreviousAttemptID, attempt.StepNo, action, attempt.Model, attempt.ChannelID, encodeStringSlice(attempt.ChannelIDs), attempt.ChannelBaseURL, attempt.ChannelName, attempt.StartedAt.UTC().Format(time.RFC3339Nano), finished, firstByte, result, attempt.FailureClass, attempt.StatusCode, redact(attempt.ErrorMessage), redact(attempt.ErrorBody), attempt.Duration.Milliseconds(), boolToInt(attempt.Stream), attempt.PromptTokens, attempt.CompletionTokens, attempt.CachedTokens, metadata, attempt.RequestLogID)
	if err != nil {
		return 0, err
	}
