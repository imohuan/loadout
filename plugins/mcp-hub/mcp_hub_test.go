...
// TestStatusOverviewDescription 验证分类总览的分类描述：
// 默认分类 = 来源 MCP 名，取服务器配置的 Description；技能分类有固定默认描述；
// 未设置描述的服务器不产生描述。
func TestStatusOverviewDescription(t *testing.T) {
	env := newHubEnv(t)

	// 给 github 服务器设置描述，weather 保持为空。
	var servers []types.MCPServer
	if err := env.st.Read(types.FileMCPServers, &servers); err != nil {
		t.Fatalf("读取 mcp_servers.json 失败: %v", err)
	}
	for i := range servers {
		if servers[i].Name == "github" {
			servers[i].Description = "GitHub 代码检索工具"
		}
	}
	...
	out, err := env.svc.Status("/mcp/$smart", nil)
	if err != nil {
		t.Fatalf("Status 失败: %v", err)
	}
	var ov struct {
	...
	want := map[string]string{
		"github":      "GitHub 代码检索工具",
		"weather":     "", // 未设置描述 → 空
		skillCategory: "已安装的技能工具",
	}
	for name, expect := range want {
		if got, ok := byName[name]; !ok {
			t.Fatalf("分类 %q 缺失，实际分类: %v", name, byName)
		} else if got != expect {
			t.Fatalf("分类 %q 描述 = %q，期望 %q", name, got, expect)
		}
	}
}
