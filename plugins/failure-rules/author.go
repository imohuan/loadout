...
func buildAuthorPrompt(sm Sample, rules string, round int, history []AuthorRound) string {
	...
	b.WriteString("\nmatch.field 可选 status_code | body_code | message_text；")
	b.WriteString("op 可选 eq（仅 status_code/body_code）| contains | not_contains | regex。")
	b.WriteString("\n需要模糊匹配且文案会变（如带 requestId、数字、动态时间）时用 message_text + regex，")
	b.WriteString("例如 {\"field\":\"message_text\",\"op\":\"regex\",\"value\":\"额度.*用尽\"}；")
	b.WriteString("正则只作用于错误文案，不要给 status_code/body_code 配 regex。")
	b.WriteString("\n【捕获模板（通用提取）】正则条件里可以用捕获组 (…) 抓值，动作里用 $1/$2… 引用：")
	b.WriteString("如正则 {\"field\":\"message_text\",\"op\":\"regex\",\"value\":\"将在 (\\\\d{4}-\\\\d{2}-\\\\d{2} \\\\d{2}:\\\\d{2}:\\\\d{2})\"} 命中后，")
	b.WriteString("action 里配 \"recover_at_template\": \"$1\" → 恢复点 = 捕获到的时间；")
	b.WriteString("配 \"cooldown_seconds_template\": \"$1\" → 冷却秒数 = 捕获的数字（如 retry after (\\\\d+) seconds）。")
	b.WriteString("捕获失败时回退 cooldown_seconds 静态值。$0 是整个匹配，$1 是第一个括号组。")
