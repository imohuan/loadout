...
func buildAuthorPrompt(sm Sample, rules string, round int, history []AuthorRound) string {
	...
	b.WriteString("\n如果错误文案明确给出了恢复/重置时刻（如「将在 2026-09-23 15:48:27 UTC+8 重置」），")
	b.WriteString("在 action 里加 \"extract_recover_at\": true——系统会自动把文案里的时间提取出来作为恢复点，")
	b.WriteString("此时 cooldown_seconds 可以省略（仅作为提取失败时的回退）。")
