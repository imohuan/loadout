...
// Emit 触发事件（与真实 Context 一致：顺序调用所有 handler，忽略返回值与错误）。
// 原为空实现，订阅 Emit 类事件（ProxyUpstreamSucceeded/Failed）的测试无法捕获载荷；
// 改为触发后需保证测试不订阅会被 Emit 的 Waterfall 事件（现有测试均无冲突）。
...
// TestUnavailableReasonReadsModelStates 回归：聚合目标不可用时，proxyRejectedLog
// 应通过 unavailableReason 从 model_states 带出真实原因（如"模型免费额度用完"），
// 而不是笼统的"当前不可用"。
func TestUnavailableReasonReadsModelStates(t *testing.T) {
	...
	if _, err := database.Exec(`INSERT INTO channels(id, name, base_url, manual_enabled, sync_billing, created_at, updated_at) VALUES
		('ch-x', 'X', 'http://x', 1, 1, 'now', 'now'),
		('ch-y', 'Y', 'http://y', 1, 1, 'now', 'now'),
		('ch-z', 'Z', 'http://z', 1, 1, 'now', 'now')`); err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	// 两种场景：额度耗尽（有 last_error）与纯冷却（无 last_error）。
	if _, err := database.Exec(`INSERT INTO model_states(channel_id, model, manual_enabled, status, disabled_until, last_error, last_failure_class, last_checked_at, updated_at) VALUES
		('ch-x', 'model-x', 1, 'cooling', ?, '模型免费额度用完', 'free_quota_exhausted', ?, ?),
		('ch-y', 'model-y', 1, 'cooling', ?, '', '', ?, ?)`, now, now, now, now, now, now); err != nil {
		t.Fatal(err)
	}
	...
	cases := []struct {
		...
		{name: "Key 多选场景：ChannelID 空 + ChannelIDs 命中", model: "model-x", channelIDs: []string{"", "ch-x", "ch-other"}, want: "模型免费额度用完"},
		{name: "无记录回退空串", model: "ghost", channelIDs: []string{"ch-z"}, want: ""},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if got := svc.unavailableReason(c.model, c.channelIDs); got != c.want {
				t.Fatalf("unavailableReason(%q, %v) = %q, want %q", c.model, c.channelIDs, got, c.want)
			}
		})
	}
	// 空 channelIDs 短路 + database 为 nil 短路
	t.Run("空 channelIDs 短路", func(t *testing.T) {
		if got := svc.unavailableReason("model-x", nil); got != "" {
			t.Fatalf("unavailableReason with empty channelIDs = %q, want empty", got)
		}
	})
	t.Run("缺 database 短路", func(t *testing.T) {
		noDBSvc, _ := newTestService(t) // 不调 SetRoutingServices，database=nil
		if got := noDBSvc.unavailableReason("model-x", []string{"ch-x"}); got != "" {
			t.Fatalf("unavailableReason with nil database = %q, want empty", got)
		}
	})
}

// TestProxyRejectedLogCarriesUnavailableReason 端到端：before-upstream 拒绝时，
// skipped attempt 的 error_message 应包含 model_states 中的真实原因。
func TestProxyRejectedLogCarriesUnavailableReason(t *testing.T) {
	database, err := db.Open(t.TempDir() + "/loadout.db")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := database.Exec(`INSERT INTO channels(id, name, base_url, manual_enabled, sync_billing, created_at, updated_at) VALUES
		('ch-x', 'X', 'http://x', 1, 1, 'now', 'now'),
		('ch-y', 'Y', 'http://y', 1, 1, 'now', 'now')`); err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	if _, err := database.Exec(`INSERT INTO model_states(channel_id, model, manual_enabled, status, disabled_until, last_error, last_failure_class, last_checked_at, updated_at) VALUES
		('ch-x', 'model-x', 1, 'cooling', ?, '模型免费额度用完', 'free_quota_exhausted', ?, ?)`, now, now, now); err != nil {
		t.Fatal(err)
	}

	svc, _ := newTestService(t)
	log := &mockRouteLog{}
	svc.SetRoutingServices(database, nil, log)
	...
	if len(log.attempts) != 2 {
		t.Fatalf("应写入 2 条 skipped attempt，实际 %d", len(log.attempts))
	}
	// 命中 model_states 的目标带出真实原因；未命中的保持笼统文案。
	if !strings.Contains(log.attempts[0].ErrorMessage, "模型免费额度用完") {
		t.Fatalf("attempt[0] error_message 应包含额度原因，实际 %q", log.attempts[0].ErrorMessage)
	}
	if !strings.Contains(log.attempts[1].ErrorMessage, "当前不可用") {
		t.Fatalf("attempt[1] error_message 应保持默认文案，实际 %q", log.attempts[1].ErrorMessage)
	}
}

// TestHandleModelsV2 v2/models：渠道模型输出 ChannelName/model，同模型跨渠道全保留，
// 虚拟模型原名，同组多 Key 只输出一次。
func TestHandleModelsV2(t *testing.T) {
	svc, st := newTestService(t)
	if err := st.Write(types.FileChannels, []types.Channel{
		{ID: "c1", Name: "Key1", ChannelName: "newapi", BaseURL: "http://newapi/v1", Enabled: true, Models: []string{"gpt-4o", "deepseek-chat"}},
		{ID: "c2", Name: "Key2", ChannelName: "newapi", BaseURL: "http://newapi/v1", Enabled: true, Models: []string{"gpt-4o"}},
		{ID: "c3", Name: "Key3", ChannelName: "bailian", BaseURL: "http://bailian/v1", Enabled: true, Models: []string{"gpt-4o", "qwen-max"}},
	}); err != nil {
		t.Fatalf("写渠道表失败: %v", err)
	}
	if err := st.Write(types.FileAggregates, []types.Aggregate{
		{Name: "auto", Models: []string{"gpt-4o"}},
	}); err != nil {
		t.Fatalf("写聚合表失败: %v", err)
	}
	...
	for w := range want {
		if !ids[w] {
			t.Fatalf("应包含 %q，实际 %+v", w, ids)
		}
	}
	// 同组多 Key 同模型只输出一次：newapi/gpt-4o 只出现一次。
	count := 0
	for _, d := range parsed.Data {
		if d.ID == "newapi/gpt-4o" {
			count++
		}
	}
	if count != 1 {
		t.Fatalf("newapi/gpt-4o 出现 %d 次，期望 1 次", count)
	}
	// 不能出现裸模型名（v1 风格）。
	if ids["gpt-4o"] {
		t.Fatalf("不应输出裸模型名 gpt-4o，实际 %+v", ids)
	}
}

// TestHandleModelsV2KeyRestriction v2/models key 白名单：裸名与带前缀名均命中。
func TestHandleModelsV2KeyRestriction(t *testing.T) {
	svc, st := newTestService(t)
	if err := st.Write(types.FileChannels, []types.Channel{
		{ID: "c1", Name: "Key1", ChannelName: "newapi", BaseURL: "http://newapi/v1", Enabled: true, Models: []string{"gpt-4o", "deepseek-chat"}},
	}); err != nil {
	...
	req := httptest.NewRequest(http.MethodGet, "/v2/models", nil)
	ctx := gatewaykeys.ContextWithAPIKey(req.Context(), types.APIKey{
		Models: []string{"gpt-4o"}, // 白名单存裸名
	})
	...
	if !ids["newapi/gpt-4o"] {
		t.Fatalf("白名单裸名 gpt-4o 应放行 newapi/gpt-4o，实际 %+v", ids)
	}
	if ids["newapi/deepseek-chat"] {
		t.Fatalf("白名单不应放行 newapi/deepseek-chat，实际 %+v", ids)
	}
}
