...
func (s *Service) Describe(ctx context.Context, images []string, viaModel, channelID string, streamWriter func(string) error) (text string, chID string, err error) {
	...
	if config.VisionCacheEnabled {
		if text, ok := s.readCache(key); ok {
			// 缓存命中：静默替换，不再向对话流输出「图片理解」。
			// 否则 agent 多轮对话每轮都带同一图片时，缓存命中会重播一遍完整描述，
			// 用户看到的就是"每轮都在调图片识别"。首轮真识别已输出过，重播无信息增益。
...
// splitLegacyImages 把旧管线消息按图片位置分为新图（最后一条 user 消息）与旧图（其余）。
// 语义与 proxy 路径的 splitProxyGroups 一致：只有新图需要识别，旧图只读缓存/占位符。
func splitLegacyImages(messages []modelgateway.ChatMessage) (newImgs, oldImgs []string) {
	lastUserIdx := -1
	...
	for i, m := range messages {
		for _, p := range m.Content.Parts {
			if p.Type == "image_url" && p.ImageURL != "" {
				if i == lastUserIdx {
					newImgs = append(newImgs, p.ImageURL)
				} else {
					oldImgs = append(oldImgs, p.ImageURL)
				}
			}
		}
	}
...
// 图片分级：只有最后一条 user 消息的图片（新图）真调视觉模型；历史旧图只读缓存/占位符，
// 避免 agent 多轮对话每轮携带历史图片时反复识别同一批图。
func (s *Service) HandleBeforeUpstream(payload any) (any, error) {
	...
	newImgs, oldImgs := splitLegacyImages(pipe.Messages)
	if len(newImgs) == 0 && len(oldImgs) == 0 {
		return payload, nil
	}
	...
	// 检出图片是常态（agent 多轮每轮带历史图），Debug 级避免刷屏；真正识别时才 Info。
	s.lg.Debug("检测到图片", "model", model, "新图数", len(newImgs), "旧图数", len(oldImgs))
	...
	for _, opt := range options {
		...
		// 旧图：只读缓存/占位符，绝不触发视觉模型调用。
		oldText := ""
		if len(oldImgs) > 0 {
			oldText = s.resolveHistoryImages(oldImgs, viaModel)
		}
		// 新图：真识别（无新图时跳过，仅历史旧图则直接替换占位/缓存文本）。
		var newText string
		if len(newImgs) > 0 {
			t, _, _, err := s.describeViaOption(context.Background(), newImgs, opt, config.DefaultVisionModel, channels, pipe.StreamWriter)
			if err != nil {
				lastErr = err
		...
		// 合并替换文本：新图描述优先，旧图缓存描述有信息时附带。
		text := newText
		if text == "" {
			text = oldText
		} else if oldText != "" && oldText != historyPlaceholder {
			text = newText + "\n\n" + oldText
		}
