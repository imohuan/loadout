...
func TestDescribeAndCache(t *testing.T) {
	...
	text, err := svc.Describe(context.Background(), []string{"http://img/a.png"}, "qwen-vl-max", "", nil)
	if err != nil {
	...
	// 二次调用命中缓存，请求数不增加。
	text2, err := svc.Describe(context.Background(), []string{"http://img/a.png"}, "qwen-vl-max", "", nil)
	if err != nil {
	...
	text3, err := svc.Describe(context.Background(), []string{"http://img/a.png"}, "qwen-vl-max", "", nil)
	if err != nil {
