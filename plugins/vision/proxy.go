...
func (s *Service) HandleProxyBeforeUpstream(payload any) (any, error) {
	...
	// 检出图片本身是常态（agent 多轮请求每轮都带历史图），Debug 级避免刷屏；
	// 真正进入视觉识别（命中 proxy 路由并调模型）时才打 Info。
	s.lg.Debug("检测到图片", "path", pipe.Request.Path, "model", model, "图片数", len(images))
	// 渠道约束：aggregate 先于本插件执行并写入 metadata。
	...
	for idx, opt := range options {
		...
		if len(newGroup.hits) > 0 {
			newText, chID, usedKeyID, err = s.describeViaOption(context.Background(), newGroup.images, opt, config.DefaultVisionModel, channels, streamWriter)
			if err != nil {
				lastErr = err
		...
		// 识别成功：更新占位为 success。
		// 缓存命中时 chID 为空（没真调），但 attempt 必须标归属哪个渠道——用 usedKeyID 兜底，
		// 否则 UI 视觉识别条目会缺 @渠道（route-log 看到 channel_id=NULL）。
		if visionLogging {
			attemptChannelID := chID
			if attemptChannelID == "" {
				attemptChannelID = usedKeyID
			}
			s.visionAttempt(context.Background(), pipe.RequestID, stepNo, viaModel, attemptStart, time.Since(attemptStart), "success", attemptChannelID, "", len(newGroup.images))
