...
func TestCallVisionCompressesDataURI(t *testing.T) {
	...
	text, _, err := svc.callVision(context.Background(), []string{orig}, "qwen-vl-max", []modelgateway.ResolvedChannel{ch}, nil)
	if err != nil {
...
func TestCallVisionKeepsRemoteURL(t *testing.T) {
	...
	ch := modelgateway.ResolvedChannel{ID: "t", Name: "test", BaseURL: url + "/v1"}
	if _, _, err := svc.callVision(context.Background(), []string{remote}, "qwen-vl-max", []modelgateway.ResolvedChannel{ch}, nil); err != nil {
		t.Fatalf("callVision 出错: %v", err)
	}
...
func TestCallVisionUsesBuiltinPrompt(t *testing.T) {
	...
	ch := modelgateway.ResolvedChannel{ID: "t", Name: "test", BaseURL: url + "/v1"}
	if _, _, err := svc.callVision(context.Background(), []string{"data:image/png;base64,iVBORw0KGgo="}, "qwen-vl-max", []modelgateway.ResolvedChannel{ch}, nil); err != nil {
		t.Fatalf("callVision 出错: %v", err)
	}
...
func TestCallVisionTooLargeFails(t *testing.T) {
	...
	ch := modelgateway.ResolvedChannel{ID: "t", Name: "test", BaseURL: url + "/v1"}
	_, _, err := svc.callVision(context.Background(), []string{orig}, "qwen-vl-max", []modelgateway.ResolvedChannel{ch}, nil)
	if err == nil {
...
func TestCallVisionOverMaxImages(t *testing.T) {
	...
	if _, _, err := svc.callVision(context.Background(), imgs, "qwen-vl-max", []modelgateway.ResolvedChannel{ch}, nil); err == nil {
		t.Fatal("超过图片张数上限应报错")
	}
}
