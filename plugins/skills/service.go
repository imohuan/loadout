...
func (s *Service) Remove(name string) error {
	...
	return s.st.Write(types.FileSkills, out)
}
...
func (s *Service) DeletePreset(name string) error {
	...
	return s.st.Write(types.FilePresets, out)
}

// ActivePreset 读 settings.json.active_preset；文件不存在时返回空字符串。
...
// SetActivePreset 写 settings.json.active_preset。
...
// activatePreset 写 settings.json 的 active_preset 与目标平台。
...
// readSkills 读 skills.json；文件不存在视为空清单。
...
// readPresets 读 presets.json；文件不存在视为空清单。
