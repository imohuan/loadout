...
func (s *Service) SetChannelEnabled(ctx context.Context, channelID string, enabled bool) error {
	...
	if enabled {
		...
		_, err := s.db.ExecContext(ctx, `UPDATE model_states SET status='available', disabled_until=NULL, fail_count=0, last_error='', last_failure_class='', last_rule_id='', last_rule_name='', updated_at=? WHERE channel_id=? AND manual_enabled=1 AND status != 'available'`, now, channelID)
		return err
	}
	return nil
}
...
func (s *Service) CheckNow(ctx context.Context, _ bool) error {
	...
	// 渠道（Key）级的定时禁用同样要到期恢复。规则引擎的 disable_key 会把整条 Key
	// 写进 channel_states（额度用尽→次日恢复等），只恢复 model_states 会让这些 Key
	// 永远停留在 cooling，路由再也选不到它。
	if _, err := s.db.ExecContext(ctx, `UPDATE channel_states SET status='available', disabled_until=NULL, updated_at=? WHERE status='cooling' AND disabled_until IS NOT NULL AND disabled_until <= ?`, now, now); err != nil {
		return err
	}
...
type channelStatusRow struct {
	...
	LastRuleID   string // 命中并禁用该 Key 的规则（UI 溯源）
	LastRuleName string
}
...
type modelStateRow struct {
	...
	LastRuleID   string // 命中并禁用该模型的规则（UI 溯源）
	LastRuleName string
...
func (s *Service) List(ctx context.Context) ([]contracts.ChannelStatus, error) {
	...
	for _, channel := range channels {
		item := contracts.ChannelStatus{
			...
			LastRuleID:    channel.LastRuleID,
			LastRuleName:  channel.LastRuleName,
		...
		for _, model := range catalog[channel.ID] {
			...
			var lastRuleID, lastRuleName string
			if ok {
				...
				lastRuleID, lastRuleName = row.LastRuleID, row.LastRuleName
			}
...
func (s *Service) listChannelStatus(ctx context.Context) ([]channelStatusRow, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT c.id, c.name, c.channel_name, c.base_url, c.manual_enabled, c.sync_billing, COALESCE(cs.status, 'available'), cs.disabled_until, COALESCE(cs.last_failure_class, ''), COALESCE(cs.last_rule_id, ''), COALESCE(cs.last_rule_name, '') FROM channels c LEFT JOIN channel_states cs ON cs.channel_id = c.id ORDER BY c.id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.ID, &row.Name, &row.ChannelName, &row.BaseURL, &row.Manual, &row.SyncBilling, &row.Status, &row.Until, &row.LastClass, &row.LastRuleID, &row.LastRuleName); err != nil {
			return nil, err
		}
...
func (s *Service) listModelStates(ctx context.Context) (map[string]map[string]modelStateRow, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT channel_id, model, manual_enabled, status, disabled_until, fail_count, last_error, COALESCE(last_failure_class, ''), COALESCE(last_rule_id, ''), COALESCE(last_rule_name, ''), last_success_at FROM model_states`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&channelID, &model, &row.Manual, &row.Status, &row.Until, &row.FailCount, &row.LastError, &row.LastClass, &row.LastRuleID, &row.LastRuleName, &row.LastSuccess); err != nil {
			return nil, err
		}
