...
func (s *Service) DeleteModels(ctx context.Context, channelID string, models []string) error {
	...
	for _, model := range models {
		if _, err := tx.ExecContext(ctx, `DELETE FROM channel_models WHERE channel_id = ? AND model = ? AND source = 'manual'`, channelID, model); err != nil {
			return fmt.Errorf("model-health: delete model catalog: %w", err)
		}
		if _, err := tx.ExecContext(ctx, `DELETE FROM model_states WHERE channel_id = ? AND model = ?`, channelID, model); err != nil {
			return fmt.Errorf("model-health: delete model state: %w", err)
		}
	}
	return tx.Commit()
...
func (s *Service) listModelCatalog(ctx context.Context) (map[string][]catalogEntry, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT channel_id, model, MAX(source) AS source FROM (
			SELECT channel_id, model, source FROM channel_models
			UNION ALL
			SELECT channel_id, model, '' AS source FROM model_states
