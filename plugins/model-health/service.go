...
func (s *Service) SetChannelEnabled(ctx context.Context, channelID string, enabled bool) error {
	_, err := s.db.ExecContext(ctx, `UPDATE channels SET manual_enabled = ?, updated_at = ? WHERE id = ?`, enabled, time.Now().UTC().Format(time.RFC3339Nano), channelID)
	return err
}
...
func (s *Service) List(ctx context.Context) ([]contracts.ChannelStatus, error) {
	...
	for _, channel := range channels {
		...
		for _, model := range catalog[channel.ID] {
			row, ok := states[channel.ID][model]
			manual := true
...
func (s *Service) listModelCatalog(ctx context.Context) (map[string][]string, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT channel_id, model FROM channel_models UNION SELECT channel_id, model FROM model_states ORDER BY channel_id, model`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		catalog[channelID] = append(catalog[channelID], model)
	}
