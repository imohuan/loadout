...
async function send() {
  ...
  // 渠道名称快照：生成日志时从 channels 取，Key 后续被删除也能兜底显示「@渠道名(Unknown)」。
  ...
  const entry: RouteLog = {
    ...
    final_channel_name: channelName,
...
async function syncTestLog(entry: RouteLog, summary: RouteLog | null) {
  ...
  try {
    ...
    if (index >= 0) {
      ...
      saveTestRouteLog(logs.value[index])
    } else {
      const withSnapshot = {
        ...detail,
        final_channel_name: detail.final_channel_name ?? entry.final_channel_name,
      ...
      logs.value.unshift(withSnapshot)
      saveTestRouteLog(withSnapshot)
    }
  } catch {
