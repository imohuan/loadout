...
const data = computed(() => {
  ...
  if (!filters.value.model && filters.value.manual_enabled === undefined && !filters.value.status) {
    return items
  }
  return items
    .map((channel) => {
      ...
      // 如果该渠道下模型被全部过滤掉，且用户确实设了模型搜索条件，则隐藏整个渠道
      if (filteredModels.length === 0 && filters.value.model) return null
