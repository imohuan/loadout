...
// channel_name 为日志快照：Key 已被删除时兜底显示「@渠道名(Unknown)」。
function channelRefFor(attempt: RouteAttempt): {
  ...
  if (attempt.channel_base_url) {
    return { channel_base_url: attempt.channel_base_url, channel_name: attempt.channel_name }
  }
  if (attempt.channel_ids?.length) {
    return { channel_ids: attempt.channel_ids, channel_name: attempt.channel_name }
  }
  return { channel_id: attempt.channel_id || '', channel_name: attempt.channel_name }
}
...
function finalChannelRef(log: RouteLog): {
  ...
  if (log.final_channel_base_url) {
    return {
      channel_base_url: log.final_channel_base_url,
      channel_name: log.final_channel_name,
    }
  }
  if (log.final_channel_ids?.length) {
    return { channel_ids: log.final_channel_ids, channel_name: log.final_channel_name }
  }
  if (log.final_channel_id && log.final_channel_id !== BUILTIN_CHANNEL) {
    return { channel_id: log.final_channel_id, channel_name: log.final_channel_name }
  }
  return null
}
...
// 仅供 running 态实时展示（终态有 duration_ms 总耗时兜底，且"估算"措辞只用于运行中）。
