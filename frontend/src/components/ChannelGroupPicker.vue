...
function applySelection(ids: Set<string>, auto: boolean) {
  ...
  if (props.channelsOnly) {
    // channelsOnly 模式：自动路由不开放，无勾选 = 清空 channel_ids；单选模式只保留第一个（传入 set 里就是唯一）。
    m.channel_id = ''
    m.channel_ids = props.multiSelect ? ordered : ordered.slice(-1)
    m.channel_base_url = ''
    m.channel_base_urls = []
  } else if (auto || ids.size === 0) {
    m.channel_id = ''
    m.channel_ids = []
    m.channel_base_url = ''
  } else {
