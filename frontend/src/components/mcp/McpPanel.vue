...
function openGroupDialog(group?: (typeof mcp.groups)[number]) {
  ...
  if (group) {
    ...
    for (const item of group.tools || []) if (ids.has(item.server_id)) expandedGroupServers.value.push(item.server_id)
  }
...
async function copyConfig(endpoint: { path: string; kind: string; label: string; transport: string }) {
...
<template>
  <div class="space-y-6">
    <PageHeader title="MCP 管理" description="配置上游 MCP、工具分组与可复制的连接端点"><template #actions><Button variant="outline"
          :disabled="mcp.refreshing" @click="mcp.refresh">
          <RiRefreshLine size="16" :class="mcp.refreshing ? 'animate-spin' : ''" />刷新
        </Button><Button v-if="activeTab === 'upstream'" @click="openServerDialog()">
          <RiAddLine size="16" />添加上游 MCP
        </Button><Button v-else-if="activeTab === 'groups'" @click="openGroupDialog()">
      ...
      <TabsList class="inline-flex h-auto w-fit max-w-full flex-wrap justify-start gap-1">
        ...
        <Card class="rounded-md">
          ...
          <CardContent class="p-0">
              ...
              <Table class="table-fixed w-full">
                ...
                <TableBody>
                  <template v-for="server in mcp.servers" :key="server.id">
                    <TableRow>
                      <TableCell class="px-0"><Button variant="ghost" size="icon" class="size-8"
                          :aria-label="isServerExpanded(server.id) ? '收起工具' : '展开工具'"
                          :aria-expanded="isServerExpanded(server.id)" @click="toggleServer(server.id)">
                      ...
                      <TableCell>
                        <div class="flex justify-end gap-1">
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger as-child><Button variant="ghost" size="icon" aria-label="测试"
                            ...
                            <Tooltip>
                              <TooltipTrigger as-child><Button variant="ghost" size="icon" aria-label="切换状态"
                            ...
                            <Tooltip>
                              <TooltipTrigger as-child><Button variant="ghost" size="icon" aria-label="删除"
                    ...
                    <TableRow v-if="isServerExpanded(server.id)" class="bg-muted/30 hover:bg-muted/30">
                      <TableCell :colspan="5" class="whitespace-normal p-0 w-full overflow-hidden">
                        <div class="space-y-3 px-4 py-4">
                          <div class="flex items-center justify-between gap-3">
                            ...
                            <Badge variant="outline" class="shrink-0">{{ serverTools(server.id).length }} 个工具</Badge>
                          </div>
                          <div v-if="serverTools(server.id).length" class="divide-y rounded-md border bg-background">
                            <div v-for="tool in serverTools(server.id)" :key="tool.name"
                              ...
                              <div class="min-w-0 flex-1 overflow-hidden">
                                ...
                                <div class="mt-1 line-clamp-2 break-words text-xs leading-5 text-muted-foreground">
                                  {{ tool.description || '暂无描述' }}
                                </div>
                              </div>
                              <div class="flex shrink-0 gap-2">
                                <Button variant="outline" size="sm" @click="openToolTest(server, tool)">
                                  <RiTestTubeLine size="16" />测试
                                </Button><Button variant="ghost" size="sm" @click="openToolEdit(server, tool)">
                                  <RiEditLine size="16" />编辑
                                </Button>
                              </div>
                            </div>
                          </div>
                          <EmptyState v-else title="没有工具" description="该服务器当前没有返回可用工具。" />
                        </div>
                      </TableCell>
                    </TableRow>
                  </template>
            ...
            <EmptyState v-else title="没有上游 MCP" description="添加一个 HTTP、SSE 或 stdio 服务开始使用。" />
          </CardContent>
        </Card>
      </TabsContent>
                  ...
                  <TableRow v-for="group in mcp.groups" :key="group.name">
                    ...
                    <TableCell class="text-right">
                      <div class="flex justify-end gap-1">
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger as-child><Button variant="ghost" size="icon" aria-label="删除分组"
      ...
      <TabsContent value="endpoints" class="space-y-4">
        <Card class="rounded-md">
          <CardHeader>
            ...
            <CardDescription>复制地址给兼容 MCP 的客户端，并在对应端点配置认证密钥。</CardDescription>
          </CardHeader>
          <CardContent class="p-0">
            <Table>
              <TableHeader>
                ...
                <template v-for="endpoint in mcp.endpoints" :key="endpoint.path">
                  <TableRow>
                    <TableCell class="w-10 pr-0">
                      <Button variant="ghost" size="icon" class="size-8" :aria-label="isEndpointExpanded(endpoint.path) ? '收起端点详情' : '展开端点详情'
                        " :aria-expanded="isEndpointExpanded(endpoint.path)" @click="toggleEndpoint(endpoint.path)">
                    ...
                    <TableCell>
                      <Badge :variant="mcp.endpointHasKey(endpoint.path) ? 'default' : 'outline'">
                        <RiKey2Line v-if="mcp.endpointHasKey(endpoint.path)" size="12" class="mr-1" />
                    ...
                    <TableCell>
                      <div class="flex justify-end gap-2">
                        ...
                        <Button v-if="mcp.endpointHasKey(endpoint.path)" variant="destructive" size="sm"
                          :disabled="keyPending" @click="deleteMcpKey(endpoint.path)">
                          <RiDeleteBinLine size="16" />删除认证
                        </Button>
                        <Button v-else variant="secondary" size="sm" :disabled="keyPending"
                  ...
                  <TableRow v-if="isEndpointExpanded(endpoint.path)" class="bg-muted/30 hover:bg-muted/30">
                      ...
                      <div class="grid gap-4 px-4 py-4 md:grid-cols-2">
                        <div class="space-y-3">
                          <div>
                            ...
                            <code
                              class="mt-1 block break-all rounded bg-background px-2 py-1 font-mono text-xs">{{ endpointUrl(endpoint.path) }}</code>
                          </div>
                          <div>
                            <div class="text-xs font-medium text-muted-foreground">认证 Header</div>
                            <code
                              class="mt-1 block rounded bg-background px-2 py-1 font-mono text-xs">X-Loadout-Key</code>
                          </div>
                          <div>
                            <div class="text-xs font-medium text-muted-foreground">认证状态</div>
                            <Badge :variant="mcp.endpointHasKey(endpoint.path) ? 'default' : 'secondary'">{{
                        ...
                        <div class="space-y-3">
                          ...
                          <div v-if="endpointKeys[endpoint.path]" class="rounded-md border bg-background p-3">
                            <div class="mb-2 flex items-center justify-between gap-2">
                              <span class="text-xs text-muted-foreground">刚刚创建的 Token（仅本次可见）</span>
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger as-child><Button variant="ghost" size="icon" class="size-8"
                                      aria-label="复制 Token" @click="copyKey(endpointKeys[endpoint.path])">
      ...
      <DialogContent class="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl!">
        ...
        <div class="space-y-4">
          <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div class="space-y-1">
              <Label>传输</Label><Select v-model="mcp.form.transport">
                <SelectTrigger>
          ...
          <div v-if="mcp.form.transport !== 'stdio'" class="space-y-3">
            <div class="space-y-1">
              <Label>URL</Label><Input v-model="mcp.form.url" placeholder="http://127.0.0.1:8000/mcp" />
            </div>
            <div class="space-y-2">
              <Label>Headers</Label>
              <div v-for="(row, index) in mcp.headers" :key="index" class="flex gap-2">
                <Input v-model="row.key" placeholder="名称" /><Input v-model="row.value" placeholder="值" />
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child><Button variant="ghost" size="icon" aria-label="删除 Header"
          ...
          <div v-else class="space-y-3">
            ...
            <div class="space-y-2">
              ...
              <div v-for="(row, index) in mcp.env" :key="index" class="flex gap-2">
                <Input v-model="row.key" placeholder="KEY" /><Input v-model="row.value" placeholder="值" />
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child><Button variant="ghost" size="icon" aria-label="删除变量"
        ...
        <DialogFooter><Button :disabled="mcp.pending" @click="saveServer">
            保存
          </Button><Button variant="outline" :disabled="mcp.pending" @click="testServerInDialog">
            测试连接
          </Button><Button variant="ghost" @click="serverDialog = false">取消</Button></DialogFooter>
      </DialogContent>
    </Dialog>
    <Dialog v-model:open="groupDialog">
      <DialogContent class="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-4xl!" @pointer-down-outside.prevent
        @interact-outside.prevent @escape-key-down.prevent>
        <DialogHeader>
        ...
        <div class="space-y-4">
          ...
          <div v-if="mcp.tools.length" class="space-y-2">
            ...
            <div class="max-h-[55vh] space-y-2 overflow-y-auto rounded-md border p-2">
              <div v-for="entry in mcp.tools" :key="entry.id" class="rounded-md border bg-background">
                <div class="flex items-center justify-between gap-2 px-2 py-2">
                  <button type="button" class="flex min-w-0 flex-1 items-center gap-2 text-left"
                    ...
                    :aria-expanded="isGroupServerExpanded(entry.id)" @click="toggleGroupServer(entry.id)">
                    <RiArrowDownSLine v-if="isGroupServerExpanded(entry.id)" size="16" class="shrink-0" />
                    <RiArrowRightSLine v-else size="16" class="shrink-0" />
                    <span class="truncate font-medium">{{ entry.name }}</span>
                    <Badge v-if="selectedCountForServer(entry.id) > 0" variant="default" class="shrink-0">
                      {{ selectedCountForServer(entry.id) }} / {{ entry.tools.length }}
                    </Badge>
                    <Badge v-else variant="outline" class="shrink-0">{{ entry.tools.length }} 个工具</Badge>
                  </button>
                  <button type="button"
                ...
                <div v-if="isGroupServerExpanded(entry.id)" class="border-t">
                  <div v-if="entry.tools.length" class="divide-y">
                    <label v-for="tool in entry.tools" :key="tool.name"
                      class="flex cursor-pointer gap-3 px-3 py-3 last:border-b-0 hover:bg-muted/40">
                      <Checkbox :model-value="mcp.selected(entry.id, tool.name)"
                        @update:model-value="mcp.toggleTool(entry.id, tool.name)" />
                      <span class="min-w-0 flex-1">
                        <span class="block truncate font-mono text-sm">{{ tool.name }}</span>
                        <span class="mt-1 block line-clamp-2 break-words text-xs leading-5 text-muted-foreground">{{
                          tool.description || '暂无描述' }}</span>
                      </span>
                    </label>
                  </div>
                  <div v-else class="px-3 py-3 text-xs text-muted-foreground">
                    该上游暂无工具。
                  </div>
                </div>
              </div>
        ...
        <DialogFooter><Button :disabled="mcp.pending" @click="saveGroup">
            <RiCheckLine size="16" />保存分组
          </Button><Button variant="outline" @click="groupDialog = false">取消</Button></DialogFooter>
      </DialogContent>
    </Dialog>
    <Dialog v-model:open="testDialog">
      <DialogContent class="flex max-h-[calc(100dvh-2rem)] flex-col overflow-hidden sm:max-w-2xl!">
          ...
          <div v-if="schemaProperties().length" class="space-y-4">
            <div v-for="[name, schema] in schemaProperties()" :key="name" class="space-y-1.5">
              ...
              <div v-else-if="schema.type === 'boolean'" class="flex items-center gap-2">
                <Checkbox :model-value="!!toolArgs[name]" @update:model-value="setToolBoolean(name, $event)" /><span
        ...
        <DialogFooter><Button :disabled="toolExecuting || toolLoading || !!toolError" @click="executeTool">
           {{ toolExecuting ? '执行中...' : '执行调用' }}
          </Button><Button variant="outline" @click="testDialog = false">关闭</Button></DialogFooter>
      </DialogContent>
    </Dialog>
