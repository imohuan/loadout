...
const props = defineProps<{ channel?: Channel; pending?: boolean }>()
  ...
  <Dialog v-model:open="open">
    <DialogContent class="sm:max-w-2xl!">
      <DialogHeader>
        ...
        <DialogDescription>支持 NewAPI 和 OpenAI 兼容的上游服务。</DialogDescription>
      </DialogHeader>
      <form class="grid gap-4 md:grid-cols-2" @submit.prevent="submit">
        <div class="space-y-2">
          <Label for="channel-name">名称</Label
          ><Input id="channel-name" v-model="form.name" required placeholder="本地 NewAPI" />
        </div>
        <div class="space-y-2">
          ...
          <Popover v-model:open="modelOpen">
            ...
            <PopoverContent class="w-[var(--reka-popper-anchor-width)] p-2" align="start">
              <div class="space-y-2">
                ...
                >
                  <p class="text-xs text-muted-foreground">未找到「{{ modelSearch }}」</p>
                  <Button
                    type="button"
                    size="sm"
                    variant="outline"
        ...
        <DialogFooter class="md:col-span-2"
          ><Button type="submit" :disabled="pending">{{ pending ? '正在保存' : '保存渠道' }}</Button
