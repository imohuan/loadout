...
import type { McpToolRank } from '@/lib/types'

const props = defineProps<{ items: McpToolRank[]; loading?: boolean }>()
const rows = computed(() =>
...
<template>
  <Card class="rounded-md">
    <CardHeader>
      <CardTitle class="flex items-center gap-2 text-base"><RiToolsLine size="18" />工具调用排行</CardTitle>
    ...
    <CardContent class="space-y-3">
      ...
      <div v-for="row in rows" :key="row.name" class="flex items-center gap-3 rounded-md border px-3 py-2">
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="truncate text-xs text-muted-foreground">上游服务：{{ row.server_name }}</div>
        </div>
