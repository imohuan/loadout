...
// 渠道字段通过 ChannelGroupPicker 写回 item。
// 关键：对比必须按「item 引用」而不是「index」——splice 上下移动后同一 index 上的
// item 已经换成了另一个，原本对比「prev[i] vs curr[i]」会被误判成渠道变化，
// 把被移动行的 model 也清空（bug 现象：上下移动后模型字段消失）。
// 移动/新增/删除都不会触发本逻辑；只有 item 引用本身、且其渠道字段确实变了才清空。
// 不能 deep watch——toggle 渠道/模型会改 item 属性，deep watch 会把 open 重置导致 popover 闪关。
const prevChannelSig = new WeakMap<ModelChannelItem, string>()
const channelSigOf = (item: ModelChannelItem) =>
  [item.channel_id, item.channel_ids?.join(','), item.channel_base_url].join('|')

watch(
  () => model.value.map((item) => ({ item, sig: channelSigOf(item) })),
  (curr, prev) => {
    // 任何一次都先把当前快照同步进 WeakMap（保留给「数组中还没出现的旧 item」使用）。
    for (const { item, sig } of curr) prevChannelSig.set(item, sig)
    if (!props.clearModelOnChannelChange || !prev) return
    const prevMap = new Map<ModelChannelItem, string>()
    for (const p of prev) prevMap.set(p.item, p.sig)
    for (const { item, sig } of curr) {
      const old = prevMap.get(item)
      // prevMap.get 对被新增行返回 undefined，跳过；只有「同一 item 引用、且字段变了」才清空。
