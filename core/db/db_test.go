...
func TestMigrateRejectsIncompatibleHistory(t *testing.T) {
	for name, corrupt := range map[string]func(*sql.DB){
		...
		"newer database": func(database *sql.DB) {
			// 插入「比程序更新」的版本号（当前迁移数 +1）才能模拟未来版本的库。
			future := len(migrations) + 1
			if _, err := database.Exec("INSERT INTO schema_migrations(version, name, checksum, applied_at) VALUES (?, 'future', 'future', 'now')", future); err != nil {
				t.Fatal(err)
			}
		},
	} {
