...
func TestOpenAppliesSchemaAndSettings(t *testing.T) {
	...
	var tables int
	if err := database.QueryRow("SELECT count(*) FROM sqlite_master WHERE type = 'table' AND name IN ('schema_migrations', 'data_imports', 'channels', 'channel_models', 'aggregates', 'aggregate_targets', 'channel_states', 'model_states', 'route_requests', 'route_attempts')").Scan(&tables); err != nil {
		t.Fatal(err)
	}
	if tables != 10 {
		t.Fatalf("routing schema tables = %d, want 10", tables)
	}
}
...
func TestMigrateRejectsIncompatibleHistory(t *testing.T) {
	for name, corrupt := range map[string]func(*sql.DB){
		...
		"newer database": func(database *sql.DB) {
			if _, err := database.Exec("INSERT INTO schema_migrations(version, name, checksum, applied_at) VALUES (5, 'future', 'future', 'now')"); err != nil {
				t.Fatal(err)
			}
		},
	} {
