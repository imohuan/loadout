...
func TestMigrateRejectsIncompatibleHistory(t *testing.T) {
	for name, corrupt := range map[string]func(*sql.DB){
		...
		"newer database": func(database *sql.DB) {
			// 程序当前有 14 条迁移，插入 version 15 才能模拟"比程序更新"的库。
			if _, err := database.Exec("INSERT INTO schema_migrations(version, name, checksum, applied_at) VALUES (15, 'future', 'future', 'now')"); err != nil {
				t.Fatal(err)
			}
		},
	} {
