...
func replaceChannelModels(ctx context.Context, tx *sql.Tx, channelID string, models []ChannelModel) error {
	...
	for i := range models {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO channel_models (channel_id, model, source, enabled, first_seen_at, last_seen_at) VALUES (?, ?, ?, ?, ?, ?)`, channelID, model.Model, model.Source, model.Enabled, model.FirstSeenAt, model.LastSeenAt); err != nil {
