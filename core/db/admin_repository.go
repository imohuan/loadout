...
func (r *Repository) ListCapabilityRoutes(ctx context.Context) ([]types.CapabilityRoute, error) {
	rows, err := r.database.QueryContext(ctx, `SELECT capability, route, models_json, channel_ids_json, channel_base_urls_json, via_options_json, replacements_json FROM capability_routes ORDER BY position, id`)
	if err != nil {
	...
	for rows.Next() {
		...
		var modelsJSON, channelsJSON, baseURLsJSON, viaJSON, replJSON string
		if err := rows.Scan(&route.Capability, &route.Route, &modelsJSON, &channelsJSON, &baseURLsJSON, &viaJSON, &replJSON); err != nil {
...
func (r *Repository) ReplaceCapabilityRoutes(ctx context.Context, routes []types.CapabilityRoute) error {
	return r.Transaction(ctx, func(tx *sql.Tx) error {
		...
		for i := range routes {
			...
			baseURLs, err := json.Marshal(route.ChannelBaseURLs)
			if err != nil {
				return fmt.Errorf("db: marshal route channel base urls: %w", err)
			}
			...
			if _, err := tx.ExecContext(ctx, `INSERT INTO capability_routes (position, capability, route, models_json, channel_ids_json, channel_base_urls_json, via_options_json, replacements_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, i, route.Capability, route.Route, string(models), string(channels), string(baseURLs), string(via), string(repl)); err != nil {
