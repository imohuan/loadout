...
```json
[
  { "id": "srv-github", "name": "github", "description": "GitHub 代码检索工具",
    "transport": "stdio", "command": "npx", "args": ["-y", "@modelcontextprotocol/server-github"], "enabled": true },
  { "id": "srv-weather", "name": "weather", "transport": "http",
    "url": "http://127.0.0.1:8000/mcp", "enabled": true }
]
```
- `description`（可选）：该上游 MCP 的一句话描述。默认分类 = 来源 MCP 名，status 第一级目录的分类描述即取自该字段；未设置则分类描述为空。技能分类（`skill`）固定描述为"已安装的技能工具"。
