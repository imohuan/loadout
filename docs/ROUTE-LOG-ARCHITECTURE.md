...
| `first_byte_at` | TEXT NULL | 流式尝试收到响应头的时刻（TTFB，v19 增补）；配合 started_at 前端展示"等待响应 Xs → 输出中 Ys" |
| `result` | TEXT | 本次尝试结果 |
...
### 7.5 流式运行中实时进度（v19 之后）

- 后端：流式 attempt 写入 `first_byte_at`（收到响应头时刻）；`proxyStream` 每 ~500ms 节流 UPSERT 估算 `completion_tokens`（CJK ≈1 token/字、其他 ≈4 字符/token，零依赖；流末真实 usage 命中后不再覆盖）。
- 前端：running 的流式 attempt 行展示 `⏳ 等待响应 Xs → 输出中 Ys · 已输出 N tok（估算）`，3s 刷新实时跳动。
- 组件复用：`RouteLogTable` 新增 `liveProgress` prop（默认 true），`ModelTestView`（模型测试页，无实时刷新链路）显式传 `false` 关闭。
- 终态 attempt 不展示阶段拆分（有 `duration_ms` 总耗时兜底，"估算"措辞仅用于运行中）。
